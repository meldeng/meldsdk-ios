...
struct MercuryoApplePayAdapter: MeldAdapter {
    ...
    // Native stays the DEFAULT for Apple Pay, and yields only to an order positively identified as
    // another shape. Requiring `presentation == .nativeToken` instead would be a behaviour change:
    // an order whose details are sparse (no merchant id yet, or a field set this build predates)
    ...
    let presentations = [MeldAdapterPresentation("APPLE_PAY", "SYSTEM_WALLET_TOKEN", "MELD_WALLET_TOKEN")]
    ...
    func matches(_ order: MeldOrder) -> Bool {
        ...
        case .providerHosted, .vendorSdk:
            return false
        case .nativeToken, .unrecognized, .none:
            return true
        }
    }
