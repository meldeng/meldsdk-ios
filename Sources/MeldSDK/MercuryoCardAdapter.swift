...
struct MercuryoCardAdapter: MeldAdapter {
    ...
    func mount(order: MeldOrder, into host: UIView, handlers: MeldEventHandlers) throws -> MeldProviderSession {
        ...
        let session = WebViewHost(url: url, orderId: order.id, handlers: handlers,
                                  allowedOrigins: Self.allowedOrigins) { message in
    ...
    static let allowedOrigins: Set<String> = Set(
        hostsByEnvironment.values.flatMap { $0 }.map { "https://\($0)" })

    private static let log = Logger(subsystem: "io.meld.sdk", category: "MercuryoCardAdapter")
    ...
    private func warnIfEnvironmentMismatch(widgetHost: String?) {
        ...
        Self.log.warning("""
            order environment is '\(orderEnv.rawValue, privacy: .public)' but Meld.configure set \
            '\(Meld.environment.rawValue, privacy: .public)'. Configure the matching environment to silence this.
            """)
    }
}
