...
struct WidgetScreen: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                if let applePay, !Meld.capabilities(for: order).embeddable {
                    ApplePayHost(order: order, request: applePay, events: events, onClose: onClose)
                        .frame(maxWidth: .infinity, maxHeight: .infinity)
                } else {
                    WidgetContainer(order: order, applePay: applePay, events: events, onClose: onClose)
                        .frame(maxWidth: .infinity, maxHeight: .infinity)
                }
            ...
            .navigationTitle(applePay == nil ? providerName.capitalized : "\(providerName.capitalized) · Apple Pay")
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        do {
            ...
            context.coordinator.handle = try Meld.mount(
                order, into: container, applePay: applePay, handlers: context.coordinator.eventHandlers())
        } catch {
