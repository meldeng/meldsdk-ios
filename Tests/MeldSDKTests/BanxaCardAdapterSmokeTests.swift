...
final class BanxaCardAdapterSmokeTests: XCTestCase {
    ...
    /// Skips where WKWebView cannot run a page at all.
    ///
    /// These are smoke tests in a unit-test target, and their point is that the real 1.3MB bundle
    /// *executes* in a real WKWebView. A headless CI runner gives the web content process no GPU —
    /// the log carries `ProcessAssertion::acquireSync Failed to acquire RBS assertion 'GPUProcess
    /// Background Assertion'` — so nothing executes, the expectation times out, and it reports as
    /// "the bundle did not execute": precisely the regression these exist to catch. A skip says which
    /// of the two it was.
    ///
    /// The capability is measured, not inferred, because two attempts at inferring it both failed:
    /// probing the network skipped nothing (the runner has network, that was never the problem), and
    /// reading `CI` / a `TEST_RUNNER_`-forwarded variable skipped nothing either (the runner's
    /// environment does not reach a process on the simulator). What matters is whether a page can run
    /// JavaScript, so that is what this asks — with a trivial inline page, no network and no bundle,
    /// on a short timeout. If that cannot round-trip, nothing downstream can, and a failure here would
    /// carry no information about the adapter.
    private func skipWhereWebViewCannotRender() throws {
        let window = UIWindow(frame: CGRect(x: 0, y: 0, width: 200, height: 200))
        let host = UIView(frame: window.bounds)
        window.addSubview(host)
        window.makeKeyAndVisible()

        let ran = expectation(description: "a trivial page can execute javascript")
        let session = WebViewHost(
            url: BanxaWebCheckoutPresenter.pageOrigin(),
            orderId: "capability-probe",
            handlers: MeldEventHandlers(),
            allowedOrigins: BanxaWebCheckoutPresenter.allowedOrigins,
            htmlContent: """
            <!doctype html><html><body><script>
            try { window.meldSendToNativeApp({kind:'message', data:{type:'alive'}}); } catch(e) {}
            </script></body></html>
            """
        ) { message in
            if (message["type"] as? String) == "alive" { ran.fulfill() }
            return []
        }
        session.mount(into: host)
        let outcome = XCTWaiter().wait(for: [ran], timeout: 10)
        session.unmount()

        try XCTSkipUnless(
            outcome == .completed,
            "WKWebView cannot execute a page here (no GPU process on a headless runner), so the "
                + "bundle smoke tests would time out and report a regression that is not there"
        )
    }
    ...
    func testDiagnoseBundleExecutionInWebView() throws {
        try skipWhereWebViewCannotRender()
    ...
    func testVendoredBundleExecutesAndCustomElementUpgrades() throws {
        ...
        let base = BanxaWebCheckoutPresenter.bootstrapHtml(bundleJs: bundle, themeCss: try BanxaWebCheckoutPresenter.loadTheme(), clientToken: "not-a-real-primer-client-token")
